"""
Generate America Myths Series - 3 Videos

1. Washington Cherry Tree (colonial_portrait style)
2. Wild West Wasn't Wild (vintage_sepia style)
3. Thanksgiving Turkey Myth (americana style)
"""
...
The real Wild West's biggest danger wasn't outlaws. It was boredom. And maybe dysentery.""",
        "image_prompts": [
            {
                "segment": "bg_1",
                "prompt": "Vintage sepia photograph style, dramatic movie poster aesthetic, cowboy silhouette with gun drawn, text THE WILD WEST crumbling and dissolving, film grain texture, 1880s daguerreotype, 9:16 vertical",
            ...
            {
                "segment": "bg_4",
                "prompt": "Vintage sepia photograph, sheriff standing next to wooden barrel full of collected pistols, sign reading NO FIREARMS, 1880s frontier town, law enforcement, historical documentary style, 9:16 vertical",
            ...
            {
                "segment": "bg_6",
                "prompt": "Vintage sepia style circus poster, Buffalo Bill Wild West Show advertisement, dramatic illustrations, the origin of western myths, entertainment marketing aesthetic, aged paper texture, 9:16 vertical",
...
So this Thanksgiving, remember: you're not following a Pilgrim tradition. You're following a 19th century magazine editor's recipe recommendations. Traditions sometimes start with good marketing.""",
        "image_prompts": [
            ...
            {
                "segment": "bg_5",
                "prompt": "Vintage 1800s advertisement poster style, colorful turkey dinner promotion, Victorian era marketing aesthetic, decorative borders, aged paper texture, commercial advertising illustration, warm autumn colors, 9:16 vertical",
                "fallback_keyword": "vintage turkey advertisement poster 1800s",
            },
            {
                "segment": "bg_6",
                "prompt": "Americana illustration style, beautiful golden roasted turkey dinner on elegant table, small ribbon banner reading TRADITION SINCE 1863, warm satisfied atmosphere, knowing wink aesthetic, warm thanksgiving colors, 9:16 vertical",
...
async def generate_video(video_config: dict, generator: ShortVideoGenerator, optimizer: ShortsOptimizer):
    """Generate a single video."""
    topic = video_config["topic"]
    style = video_config["style"]
    script = video_config["script"]
    image_prompts = video_config["image_prompts"]
    ...
    config = GenerationConfig(
        ...
        series_override=series_info.series_name,
    )
...
async def main():
    """Generate all America Myths videos."""
    print("=" * 60)
    print("America Myths Series Generator")
    print("=" * 60)
    print()
    print("Videos to generate:")
    for v in VIDEOS:
        print(f"  {v['id']}. {v['topic']} ({v['style']})")
    print()
    ...
    for video_config in VIDEOS:
        success = await generate_video(video_config, generator, optimizer)
        results.append((video_config["topic"], success))
        print()
    ...
    print(f"\nTotal: {success_count}/{len(results)} videos generated")
